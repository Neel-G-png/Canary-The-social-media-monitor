...
print("PLACE : ",status.user.translator_type)
print("USER KA FOLLOWERS COUNT : ",status.user.followers_count)
print("USER KA FRIEND COUNT : ",status.user.friends_count)
print("USER KA LISTED COUNT : ",status.user.listed_count)
print("USER KA VERIFICATION STATUS : ",status.user.verified)
...
class MyStreamListener(tweepy.StreamListener):
    ...
    def on_status(self,status):
        ...
        if status.lang == 'en':
            ...
            sql = "insert into twitter_data (id,link,tweet,username,follower_count,friend_count,listed_count,verification_status,tweet_for_user)\
                   values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            val = (self.id,self.link,self.tweet[:500],self.user_name,self.user_follower_count,self.friend_count,self.listed_count,self.verification_status,tweet_for)
